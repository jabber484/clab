<div class="col-sm-6 col-md-3">
	<div class="project-wrapper">
	    <div class="project-img">
			<img src="https://scontent-hkg3-1.xx.fbcdn.net/v/t31.0-8/15002269_1331852580158450_7900758843559021126_o.jpg?oh=d992a682ebd8e757eba8c07b64f043fb&oe=5A5043EF">
	    </div>
	    <div class="project-des">
		    <div class="project-title">I Want To Kill Your Mum</div>
		    <div class="project-date">12 Sept ~ 14 Sept</div>
		    <div class="project-detail">How does the implicit hog flip? How does the librarian conduct a crop? The buss discontinues the synonym. </div>
		    <div class="project-link">Detail</div>
		</div>
	</div>
	<div class="project-stream">
		<div class="project-stream-word">Science & Technology</div>
	</div>
</div>
<div class="col-sm-6 col-md-3">
	<div class="project-wrapper">
	    <div class="project-img">
		    <img src="{{asset('asset/home/banner/hammer_fun.jpg')}}">
	    </div>
	    <div class="project-des">
		    <div class="project-title">I Want To Kill Your Mum</div>
		    <div class="project-date">12 Sept ~ 14 Sept</div>
		    <div class="project-detail">sdfsdfsfsfsdf</div>
		    <div class="project-link">Detail</div>
	    </div>
	</div>
	<div class="project-stream">
		<div class="project-stream-word">Science & Technology</div>
	</div>
</div>
<div class="col-sm-6 col-md-3">
	<div class="project-wrapper">
	    <div class="project-img">
		    <img src="{{asset('asset/home/banner/banner1.jpg')}}">
	    </div>
	    <div class="project-des">
		    <div class="project-title">I Want To Kill Your Mum</div>
		    <div class="project-date">12 Sept ~ 14 Sept</div>
		    <div class="project-detail">sdfsdfsfsfsdf</div>
		    <div class="project-link">Detail</div>
		</div>
	</div>
	<div class="project-stream">
		<div class="project-stream-word">Science & Technology</div>
	</div>
</div>
<div class="col-sm-6 col-md-3">
	<div class="project-wrapper">
	    <div class="project-img">
		    <img src="{{asset('asset/home/banner/banner1.jpg')}}">
	    </div>
	    <div class="project-des">
		    <div class="project-title">I Want To Kill Your Mum</div>
		    <div class="project-date">12 Sept ~ 14 Sept</div>
		    <div class="project-detail">sdfsdfsfsfsdf</div>
		    <div class="project-link">Detail</div>
		</div>
	</div>
	<div class="project-stream">
		<div class="project-stream-word">Science & Technology</div>
	</div>
</div>